...
- The vertical gutter width can be changed with `.gt-*` classes like `.gt-2` (smaller vertical gutters) or `.gt-xl-4` (larger vertical gutters on viewports larger than the `xl` breakpoint). To achieve vertical gutters, additional margin is added to the top of each column. That's why `gt-*` classes are used instead of `gy-*` classes. The `.row` counteracts this margin to the top with a negative margin.
...
`.gt-*` classes can be used to control the horizontal gutter widths. Like the horizontal gutters, the vertical gutters can cause some overflow below the `.row` at the end of a page. If this occurs, you add a wrapper around `.row` with the `.overflow-hidden` class:

{{< example >}}
<div class="container overflow-hidden">
  <div class="row gt-lg-5">
    <div class="col-6">
      <div class="p-3 border bg-light">Custom column padding</div>
    </div>
    <div class="col-6">
